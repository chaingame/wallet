...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::CG, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert ChainGame:// to ChainGame:
    //
    //    Cannot handle this later, because ChainGame:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ChainGame://", Qt::CaseInsensitive))
    {
        uri.replace(0, 12, "ChainGame:");
    }
...
{
    QString ret = QString("ChainGame:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CG, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
